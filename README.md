...
- Optionally use the Shrine's normal paid transformation, for a configurable number of attempts, to seek missing guide skills. The optimizer reserves the reset cost, may continue with a clearly reported partial match, then strictly resets and concentrates points in the available relevant guide masteries.
...
Auto Build operates on the hero currently selected in the game. Gear optimization keeps currently equipped items in the candidate pool and searches both weapon slots plus the other six slots as one loadout. Job weapon rules remain hard constraints; the selected or guide-recommended base skill's weapon requirement or preference is also respected, while matches for other active skills add synergy rather than incorrectly excluding every alternative. Native attributes, rune affixes, granted skills, active Set thresholds, and guide equipment are evaluated together. The score estimates roughly 60 seconds of uninterrupted single-target output from the selected base skill using the game's native damage, cooldown, and skill-speed calculations. It assumes sufficient resources and uptime and does not fully simulate enemy defenses, movement, complex triggers, DoT stacking, summons, or combat AI. The chosen eight-slot loadout is applied as a verified transaction across Inventory, Warehouse, and unlocked Vault storage. Native storage routing normalizes committed Vault gear; any failed transfer rolls back, and a full bag with no reversible bridge causes a safe stop and restoration.

Skill optimization can use the Shrine's normal Blood-paid transformation for up to the configured number of attempts to look for missing build-guide skills. It keeps enough Blood in reserve for the reset, clearly reports a partial match if every target cannot be found, then performs a strictly validated reset and concentrates points in the relevant guide masteries that are actually available. Gear and skill actions each require a second confirmation click.
