...
- 強制遵守職業武器規則，以及所選或推薦基礎技能的武器需求／偏好；其他主動技能只計入協同評分，不會被錯誤地全部設為硬性限制
- 結合遊戲原生屬性、符文詞綴、已啟用套裝階段和技能協同，依所選基礎技能的原生傷害、冷卻與技能速度估算約 60 秒持續輸出；此分數並非精確的戰鬥 DPS 模擬
- 可在設定次數內使用 Shrine 的正常付費轉換尋找缺少的攻略技能，保留重置費用；允許回報部分符合後繼續，再嚴格重置並將點數集中到目前可用的相關攻略專精
...
自動配裝會套用到遊戲中目前選擇的英雄。它會同時搜尋八個裝備欄位，職業武器規則是硬性限制，並遵守所選或攻略推薦基礎技能的武器需求或偏好；其他主動技能則透過協同評分呈現。評分綜合原生屬性、符文詞綴、賦予技能、已啟用套裝階段和攻略裝備，並使用遊戲原生傷害、冷卻和技能速度，估算對單一目標連續攻擊約 60 秒的輸出。它假設資源與持續輸出條件充足，不會完整模擬敵方防禦、移動、複雜觸發、持續傷害疊層或召喚物 AI。選出的八件裝備會在背包、Warehouse 和已解鎖 Vault 之間以可驗證的交易方式移動，成功後按遊戲原生規則整理 Vault；任何失敗都會回復。若背包已滿且沒有可逆中轉空位，則安全停止並還原原狀。

自動技能可在設定的最大次數內使用 Shrine 的正常 Blood 付費轉換，尋找攻略中缺少的技能。它會保留重置所需 Blood；若無法取得全部目標，會清楚回報部分符合並繼續。接著執行嚴格驗證的重置，並把點數集中投入實際可用的相關攻略專精。自動裝備與自動技能操作都需要第二次確認。
